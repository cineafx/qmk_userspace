...
[_QWERTY] = LAYOUT_planck_grid(
   ...
   CTRL_ESC, KC_A   , KC_S   , KC_D   , KC_F   , KC_G   ,   KC_H   , KC_J   , KC_K   , KC_L   , KC_SCLN, KC_QUOT,
    ...
    MOD    , QK_GESC, KC_LGUI, KC_LALT, LOWER  , KC_SPC ,   KC_ENT , RAISE  , KC_BSPC, KC_ALGR, _______, SYSTEM
),

/*
 * "Gaming" mode where it doesn't use Mod-Tap. It's always a ctrl key.
 * (Second key on the bottom row is still QK_GESC from the _QUERTY layer.)
 */
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case EXT_PLV:
      ...
      CUSTOMMACRO(UWU,      send_unicode_string("ᵘwͧᵘ");)
